...
export default function WorkoutForm({
  ...
  async function handleSubmit(e) {
    ...
    setSaving(true)
    try {
      await onSubmit({ date, notes: notes.trim() || null, exercises: cleanExercises })
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className={`flex flex-col gap-6 ${warning ? '' : 'mt-4'}`}>
        ...
        <div className="flex flex-col gap-4">
          {exercises.map((ex, exIdx) => (
            <div key={exIdx} className="rounded-xl border border-slate-800 bg-slate-900/50 p-4 shadow-sm">
                ...
                {ex.sets.map((s, setIdx) => (
                  <div key={setIdx} className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="w-full rounded-lg border border-slate-800 bg-slate-900 px-3 py-2 text-sm outline-none transition-shadow focus:border-violet-500 focus:ring-4 focus:ring-violet-500/30"
                    />
                    <input
                      type="number"
                      min="0"
